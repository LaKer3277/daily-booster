    ...
    private fun formatScanCache() {
        synchronized(CleanData.cache) {
            ...
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.ram_used),
            ...
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.app_cache),
...
//            if (scanTemp.apkFiles.isNullOrEmpty().not()) {
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.apk_files),
...
//            if (scanTemp.appResidual.isNullOrEmpty().not()) {
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.app_residual),
...
//            if (scanTemp.logFiles.isNullOrEmpty().not()) {
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.log_files),
...
//            if (scanTemp.tempFiles.isNullOrEmpty().not()) {
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.temp_files),
...
//            if (scanTemp.otherFiles.pathList.isNullOrEmpty().not()) {
            DaiBooCleanParentBean(
                name = DBApp.ins.getString(R.string.ad_junks),
...
//                        LogUtils.dScan("过滤--开始-->appRegex---$appRegex")
    ...
    private fun addAppCacheItem(packageName: String, filePath: String): DaiBooCleanChildBean? {
        return try {
            val pm = DBApp.ins.packageManager
    ...
    fun initFilters(aggressive: Boolean) {
        ...
        files.addAll(DBApp.ins.resources.getStringArray(R.array.common_filter_files))
        folders.addAll(DBApp.ins.resources.getStringArray(R.array.common_filter_folders))
        if (aggressive) {
            files.addAll(DBApp.ins.resources.getStringArray(R.array.aggressive_files))
            folders.addAll(DBApp.ins.resources.getStringArray(R.array.aggressive_folders))
        }
        filters.clear()
