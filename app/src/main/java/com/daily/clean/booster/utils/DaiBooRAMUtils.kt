...
object DaiBooRAMUtils {
    ...
    fun getUsedMemory(): Long {
        val activityManager =
            DBApp.ins.getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager
    ...
    fun getUsedMemoryString(context: Context?=null): String {
        val am = DBApp.ins.getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager
        val memoryInfo = ActivityManager.MemoryInfo()
    ...
    fun getInstalledAppAmounts(callback: (Int) -> Unit) {
        ...
        callback.invoke(DBApp.ins.packageManager.queryIntentActivities(intent, 0).size)
    }
    fun getInstalledAppAmounts2():Int {
        val intent = Intent(Intent.ACTION_MAIN, null)
        intent.addCategory(Intent.CATEGORY_LAUNCHER)
       return DBApp.ins.packageManager.queryIntentActivities(intent, 0).size
    }

    private fun getInstalledPackageNameList(): List<String> {
        val intent = Intent(Intent.ACTION_MAIN, null)
        intent.addCategory(Intent.CATEGORY_LAUNCHER)
        return DBApp.ins.packageManager.queryIntentActivities(intent, 0)
